...
- (void)cl_playMiddleButtonAction:(UIButton *)button {
    if (button.selected == NO){
        [self pausePlay];
    }else{
        [self playVideo];
...
- (void)pausePlay{
    self.maskView.playButton.selected = NO;
        self.maskView.playMiddleButton.selected = NO;
    self.maskView.playMiddleButton.hidden = NO;
    [_player pause];
}
#pragma mark - 播放
- (void)playVideo{
    _isUserPlay = YES;
        ...
        self.maskView.playMiddleButton.selected = YES;
        self.maskView.playMiddleButton.hidden = YES;
