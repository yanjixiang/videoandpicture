...
- (void)initViews{
    ...
    [self addSubview:self.playMiddleButton];
...
- (void)makeConstraints{
    ...
    //中间播放按钮
    [self.playMiddleButton mas_makeConstraints:^(MASConstraintMaker *make) {
...
//中间播放按钮
- (UIButton *) playMiddleButton{
    if (_playMiddleButton == nil){
        _playMiddleButton = [[UIButton alloc] init];
        [_playMiddleButton setImage:[UIImage imageNamed:@"播放"] forState:UIControlStateNormal];
//        _playMiddleButton.backgroundColor = yRedColor;
        [_playMiddleButton addTarget:self action:@selector(playMiddleButtonAction:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _playMiddleButton;
}
//全屏按钮
- (UIButton *) fullButton{
    if (_fullButton == nil){
...
- (void)playButtonAction:(UIButton *)button{
    ...
    if (button.selected) {
        _playMiddleButton.hidden = YES;
        _playMiddleButton.selected = YES;
    }else {
        _playMiddleButton.hidden = NO;
        _playMiddleButton.selected = NO;
    }
...
//中间播放按钮
- (void)playMiddleButtonAction:(UIButton *)button {
    button.selected = !button.selected;
    if (button.selected) {
        _playMiddleButton.hidden = YES;
    }else {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(cl_playMiddleButtonAction:)]) {
        [_delegate cl_playMiddleButtonAction:button];
    }else{
        NSLog(@"没有实现代理或者没有设置代理人");
    }
}
...
- (void)failButtonAction:(UIButton *)button{
    self.playMiddleButton.hidden = YES;
...
- (void)progressSliderTouchBegan:(CLSlider *)slider{
    self.playMiddleButton.hidden = NO;
